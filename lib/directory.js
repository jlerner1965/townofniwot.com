/* Business directory: record schema, build-time validation and publication.
   ...
   Food and drink records carry six more fields, which the restaurant guide
   at /restaurants/ renders and the directory ignores. Every one of them is
   optional except `guide`, and none of them may be inferred: a facet is
   filled in only where the record's own source states it, and left off
   otherwise, so an empty facet reads as "not published" rather than "no".

   guide          slug of a group in FOOD_GROUPS — required on a published
                  record in a FOOD_CATEGORIES category, so a new kitchen
                  cannot be added without deciding where the guide files it
   cuisine        the kitchen's own description of what it cooks, condensed
   kind           what sort of room it is — sit-down, counter, taproom
   patio          outdoor seating in the source's own words; omitted where
                  the source does not mention any
   serves         what the source says about meals or days served. Not
                  opening hours: this site does not reproduce those
   menuUrl        the menu's own page, where the source publishes one (https)
   priceRange     one of $, $$, $$$, $$$$, and only from a source that
                  publishes a price range. Never estimated from a menu
...
/* The categories the restaurant guide draws from. A published record in one
   of these must name the group it belongs to. */
export const FOOD_CATEGORIES = ['restaurants-bars', 'coffee-bakery', 'grocery-provisions'];

/* How /restaurants/ groups them, in render order. The grouping is by the
   kind of meal somebody is looking for rather than by the directory's
   categories, which sort by trade. */
export const FOOD_GROUPS = [
  { slug: 'sit-down', label: 'Sit-down kitchens', note: 'Table service, indoors or on a patio.' },
  { slug: 'bars-breweries', label: 'Bars & breweries', note: 'A pint, a taproom and whatever the kitchen is doing.' },
  { slug: 'quick', label: 'Counters & quick meals', note: 'Ordered at a counter, eaten there or carried out.' },
  { slug: 'coffee-sweets', label: 'Coffee, bakery & sweets', note: 'Mornings, an afternoon stop and the walk home.' },
  { slug: 'provisions', label: 'Groceries & provisions', note: 'What to take with you, and the shelves behind the deli.' },
];
...
export function validateListings(records, { today = new Date().toISOString().slice(0, 10), freshnessDays = FRESHNESS_DAYS, categories = CATEGORIES, groups = FOOD_GROUPS } = {}) {
  const errors = [];
  const warnings = [];
  ...
  records.forEach((r, i) => {
    ...
    /* The restaurant guide's facets. `guide` is required on a published food
       record so a new kitchen cannot be added and then quietly go missing
       from /restaurants/; the rest are optional and are left off wherever the
       source is silent. */
    for (const field of ['cuisine', 'kind', 'patio', 'serves']) {
      if (r[field] !== undefined && (typeof r[field] !== 'string' || !r[field].trim())) errors.push(`${id}: ${field} must be a non-empty string`);
    }
    checkUrl(r.menuUrl, 'menuUrl', r, errors);
    if (r.priceRange !== undefined && !PRICE_RANGES.includes(r.priceRange)) {
      errors.push(`${id}: priceRange "${r.priceRange}" is not one of ${PRICE_RANGES.join(', ')} — and it is set only from a source that publishes one`);
    }
    if (r.guide !== undefined && !groupSlugs.has(r.guide)) {
      errors.push(`${id}: guide "${r.guide}" is not one of ${[...groupSlugs].join(', ')}`);
    }
    if (PUBLISHED_STATUSES.includes(r.status) && FOOD_CATEGORIES.includes(r.category) && !r.guide) {
      errors.push(`${id}: a published ${r.category} record needs a guide group (${[...groupSlugs].join(', ')}) so it appears on /restaurants/`);
    }
    if (r.guide && !FOOD_CATEGORIES.includes(r.category)) {
      errors.push(`${id}: guide is for food and drink records only, and "${r.category}" is not one of ${FOOD_CATEGORIES.join(', ')}`);
    }
...
export function publishListings(records, categories = CATEGORIES) {
  ...
  return {
    ...
    food: publishFoodGuide(entries),
  };
}

/* The restaurant guide's view of the same published rows: the food and drink
   records, in group order, each carrying the facets the guide renders and the
   anchor of its row in the full directory.

   `menuHref` is the menu's own page where the record names one and the
   business's own site otherwise, with a label that says which — the same rule
   the directory uses for its outward links. A record with neither has no
   destination of its own and links only to the directory row, which names
   where it was checked. */
export function publishFoodGuide(entries, groups = FOOD_GROUPS) {
  const rows = entries
    .filter((e) => FOOD_CATEGORIES.includes(e.category) && e.guide)
    .map((e) => ({
      ...e,
      anchor: `/eat-shop/#${e.slug}`,
      /* A street address on its own is ambiguous to a map — there is a
         Second Avenue in most towns in Colorado — so the locality goes on
         every query, and a row with no address is looked up by name. */
      directionsQuery: `${e.address || e.name}, Niwot, CO 80503`,
      menuHref: e.menuUrl || e.website || null,
      menuLabel: e.menuUrl ? 'Menu' : e.website ? 'Official site' : null,
      /* The facets this row can actually stand behind, in a fixed order, so
         a row with two says two and a row with five says five. */
      facets: [
        e.cuisine && { label: 'Cuisine', value: e.cuisine },
        e.kind && { label: 'Kind of place', value: e.kind },
        e.area && { label: 'Where', value: e.area },
        e.patio && { label: 'Outdoor seating', value: e.patio },
        e.serves && { label: 'Serves', value: e.serves },
        e.priceRange && { label: 'Price range', value: e.priceRange },
      ].filter(Boolean),
    }));

  const grouped = groups
    .map((g) => ({ ...g, entries: rows.filter((r) => r.guide === g.slug) }))
    .filter((g) => g.entries.length > 0);

  return {
    entries: rows,
    groups: grouped,
    /* The facets that are filled in often enough to be worth a filter, and
       the ones that are not, so the page can say which is which. */
    withPatio: rows.filter((r) => r.patio),
    withMenu: rows.filter((r) => r.menuUrl),
    withPrice: rows.filter((r) => r.priceRange),
